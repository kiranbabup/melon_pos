...
import BranchModal from "./superComponents/BranchModal";
...
function ManageBranchesPage() {
  ...
  const columns = [
    ...
    { field: "address", headerName: "Address", flex: 1 },
    {
      ...
      renderCell: (params) => (
        <Switch
          ...
          onChange={() =>
            toggleUsageStatus(params.row.branch_id, params.row.status)
          }
          color="primary"
        />
      ),
    ...
    //       onClick={() => {
    //         setFormData({
    //           name: params.row.name, // backend field is name
  ...
  const toggleUsageStatus = async (id, currentStatus) => {
    try {
      ...
      const response = await updateBranches(id, { status: newStatus });
