...
const Calculator = ({ paymentMethod, setPaymentMethod, grandTotal }) => {
  ...
  const onCalChange = () => {
    ...
    const total = Number(grandTotal) || 0; // 👈 use discounted total
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        sx={{ width: 200 }}
        InputProps={{
          endAdornment: amount && ( // show only if there is text
      ...
      {/* Calculator UI */}
      <Box
        sx={{ display: "grid", gridTemplateColumns: "repeat(3, 70px)", gap: 2 }}
      >
      ...
      <AccentButton fullWidth sx={{ mt: 2 }} onClick={() => onCalChange()}>
        Calculate Change
      </AccentButton>
